...
func main() {
	...
	specialForms["let*"] = sfLet
	specialForms["do"] = sfDo
	specialForms["if"] = sfIf
...
func sfLet(list []*types.Data, env *types.Env) (*types.Data, error) {
...
func sfDo(list []*types.Data, env *types.Env) (*types.Data, error) {
	parts := list[1:] // Strip off the "do".
	evald, err := eval_ast(&types.Data{List: &parts}, env)
	if err != nil {
		return nil, err
	}
	return (*evald.List)[len(*evald.List)-1], nil
}

func sfIf(list []*types.Data, env *types.Env) (*types.Data, error) {
...
func runClosure(list []*types.Data) (*types.Data, error) {
	c := *list[0].Closure
	evald, err := eval_list(list[1:], c.Env)
	if err != nil {
		return nil, err
	}
	if len(evald) < len(c.Params) {
		return nil, fmt.Errorf("Not enough parameters: expected at least %d, got %d",
				len(c.Params), len(evald))
	}
	newEnv := types.NewEnv(c.Env, c.Params, evald[0:len(c.Params)])

	if c.TailParams != "" {
		tailArgs := evald[len(c.Params):]
