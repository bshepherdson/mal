...
// Lists
func mkList(args []*types.Data) (*types.Data, error) {
...
func nth(args []*types.Data) (*types.Data, error) {
	if len(args) != 2 || args[0].List == nil || args[1].Number == nil {
		return nil, fmt.Errorf("nth expects a list and number")
	}
	idx := *args[1].Number
	list := *args[0].List
	if idx >= len(list) {
...
func first(args []*types.Data) (*types.Data, error) {
	if len(args) != 1 {
...
func rest(args []*types.Data) (*types.Data, error) {
	if len(args) != 1 || args[0].List == nil {
		return nil, fmt.Errorf("rest expects a list")
	}
	l := *args[0].List
	if len(l) == 0 {
		return args[0], nil // Empty lists are returned.
	}
	return list(l[1:]...), nil
}
