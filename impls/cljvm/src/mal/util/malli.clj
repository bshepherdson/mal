(ns mal.util.malli
  ...
  (:require
   ...
   [malli.destructure]
   [mal.util.malli.fn :as mu.fn]))

;; This namespace is borrowed from
;; https://github.com/metabase/metabase metabase.util.malli.defn
  ...
  (str/trim
   (str "Inputs: " (case arities-type
                     ...
                     :multiple (str "("
                                    ...
                                    ")"))
        "\n  Return: " (str/replace (pr-str (:schema return :any))
                                    "\n"
                                    (str "\n          "))
...
(defmacro defn
  ...
  (let [parsed           (mu.fn/parse-fn-tail fn-tail)
    ...
    `(def ~(vary-meta fn-name merge attr-map)
       ~docstring
       ~(let [error-context {:fn-name (list 'quote (symbol (name (ns-name *ns*)) (name fn-name)))}]
