  ...
  (let [parsed (mc/parse mx/SchematizedParams (if (symbol? (first fn-tail))
    ...
    (when (= parsed ::mc/invalid)
      (let [error     (mc/explain mx/SchematizedParams fn-tail)]
        (throw (ex-info (format "Invalid function tail: %s" error)
                        {:fn-tail   fn-tail
                         :error     error}))))
    parsed))
...
(defn- validate [error-context schema value error-type]
  (when *enforce*
    (when-let [error (mc/explain schema value)]
      (throw (ex-info (case error-type
                        ::invalid-input  (str "Invalid input: "  (pr-str error))
                        ::invalid-output (str "Invalid output: " (pr-str error)))
                      (merge
                        {:type      error-type
                         :error     error
                         :schema    schema
